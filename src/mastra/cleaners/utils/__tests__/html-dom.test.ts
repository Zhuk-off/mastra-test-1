...
import { hasServerTags, stripServerTags } from '../html-dom.js';
...
describe('stripServerTags — вырезать ВЕСЬ серверный код (C2, политика владельца)', () => {
  it('<?php ... ?> вырезается, разметка вокруг цела', () => {
    expect(stripServerTags('<div><?php echo $x; ?></div>')).toBe('<div></div>');
  });
  it('незакрытый <?php ... до конца файла', () => {
    expect(stripServerTags('<div></div>\n<?php echo $x;')).toBe('<div></div>\n');
  });
  it('<?= short echo', () => {
    expect(stripServerTags('<a href="<?= $u ?>">x</a>')).toBe('<a href="">x</a>');
  });
  it('незакрытый <?= до конца файла', () => {
    expect(stripServerTags('<b>hi</b><?=$t')).toBe('<b>hi</b>');
  });
  it('ASP/JSP <% ... %>', () => {
    expect(stripServerTags('<p><% Response.Write(1) %></p>')).toBe('<p></p>');
  });
  it('PHP, генерящий структуру (блоки вырезаны, текст цел)', () => {
    expect(stripServerTags('<a href="x"><?php if($c){ ?>text<?php } ?></a>')).toBe('<a href="x">text</a>');
  });
  it('одна строка spysecure сверху удаляется', () => {
    expect(stripServerTags('<?php include "spysecure.php"; ?>\n<!doctype html><html></html>')).toBe(
      '\n<!doctype html><html></html>',
    );
  });
  it('<?xml ...?> НЕ трогаем (это не серверный код)', () => {
    expect(stripServerTags('<?xml version="1.0"?><svg></svg>')).toBe('<?xml version="1.0"?><svg></svg>');
  });
  it('чистый HTML без серверных тегов — без изменений', () => {
    const html = '<!doctype html><html><body><p>hi</p></body></html>';
    expect(stripServerTags(html)).toBe(html);
  });
});
