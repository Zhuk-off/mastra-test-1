...
- **EUF-1 ✅** — `extractUsefulFunctions` стал reference-safe: если на pure-exfil функцию где-то
  ссылаются (вызов/обработчик), её ОБЪЯВЛЕНИЕ не удаляется — обнуляется ТЕЛО (`function track(){}`),
  символ жив → нет `ReferenceError`. Если ссылок нет — удаляется целиком (как раньше). Ссылки считаются
